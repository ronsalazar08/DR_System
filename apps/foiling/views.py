...
def delete_dr(request, cnum):
    ...
    try:
        return redirect(f'http://{request.get_host()}/foiling/?page={cno[1]}')
    except:
        return redirect(f'http://{request.get_host()}/foiling/?page={cno[1] - 1 }')
...
def delete_item(request, pk):
    ...
    return redirect(f'/foiling/edit_dr/{cnum.control_noFK}')
